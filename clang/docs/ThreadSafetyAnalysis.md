...
a warning since a hold has one kind. A try-acquire
of a capability that is already conditionally held by another try-acquire is
tracked separately: each is resolved by the branch on its own return value.
Asserting the capability (`ASSERT_CAPABILITY`) upgrades it to held without a
warning.
