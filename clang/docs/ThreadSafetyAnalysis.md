...
The capability is tracked as conditionally ("try") held from the call until a recognized branch on its return value: on the success path the capability is held, on the failure path it is not. A branch is recognized when it tests the call's result directly or through a local variable, including a variable that merges the result with an earlier constant initializer (`bool ok = false; if (...) ok = mu.TryLock();` and the spin-loop `while (!ok) ok = mu.TryLock();`) -- when the variable is true despite the false initializer, the try-acquire must have succeeded. A conditionally held capability does not satisfy requirements such as `GUARDED_BY` or `REQUIRES` and it also violates `LOCKS_EXCLUDED` and negative requirements (`REQUIRES(!mu)`). Acquiring it again before branching on the return value warns that it may already be held, and releasing it warns that it may not be held. A try-acquire of a capability that is already held (or conditionally held) also warns at the call: the analysis keeps one fact per capability, so the attempted acquisition cannot be tracked -- except for a reentrant capability that is definitely held, which such a call simply re-enters. Asserting the capability (`ASSERT_CAPABILITY`) upgrades it to held without a warning. Under `-Wthread-safety-beta`, a try-acquire whose result is never used to determine success additionally warns where the analysis loses track of it -- at a merge with a path that does not hold the capability, or at the end of the function -- since the capability may then be leaked.

A declaration may carry several `TRY_ACQUIRE` attributes, and they may specify different success values: each capability is acquired on the branches where its own attribute's success value matches the return value. Different capabilities under opposite success values describe a function that acquires one capability or the other depending on its result (`TRY_ACQUIRE(true, mu1) TRY_ACQUIRE(false, mu2)`).
