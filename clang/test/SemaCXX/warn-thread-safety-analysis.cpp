...
struct TestTryLock {
  ...
  // the second try-acquire is tracked beside it and resolved by its own
  ...
  // A second try-acquire of the same capability while it is try-held is
  // tracked as a second conditional fact: the calls cannot both succeed at
  ...
  // other kind while try-held is still diagnosed.
  ...
  // the unchecked try-acquire is the reason the hold is not definite.
  ...
  // As above with a re-branch on the result: the success edge is simply
  ...
  // Re-branching on a result that a promoted fact already proves succeeded:
  ...
  // it is resolved -- the capability conservatively stays try-held
  ...
  // negative fact contradicts it. Here the conditional release leaves one
  // (kept as one-sided evidence through the join): the stored result no
  // longer witnesses a live hold on that path, so the success region is
  // not resurrected and uses of the capability in it warn, with the
  // release as the note. The loss itself was already diagnosed at the
  // join (in beta mode).
  ...
  // to release is diagnosed as a reentrancy-depth mismatch at each join
  // that loses it, and as still held at the end of the function.
  void tryheld_multi_cap_over_held(bool c) {
    mu2.Lock();             // expected-note 3 {{mutex acquired here}}
    bool ok = false;
    if (c)
      ok = TryLockBoth();
    if (ok) {               // expected-warning {{mutex 'mu2' is not held on every path through here with equal reentrancy depth}}
      a = 1;
      mu.Unlock();
    }
    mu2.Unlock();           // expected-warning {{mutex 'mu2' is not held on every path through here with equal reentrancy depth}}
  ...
  // path -- only the guaranteed depth differs -- so the join draws the
  // reentrancy-mismatch wording rather than a false lost-hold warning. The
  // conditional level keeps its origin through the join and the release,
  // so the leak note points at the try-acquire.
  ...
  void tryheld_over_held_mixed_loop() {
    mu.Lock();     // expected-note {{mutex acquired here}}
    while (cond) { // expected-warning {{expecting mutex 'mu' to be held at start of each loop with equal reentrancy depth}}
      mu.TryLock();
    }
    mu.Unlock();
  }
  ...
  // hold is definite as promised: no false expecting-to-be-held -- the
  // depth diagnostic and the beta warning report the leaky try level.
  void tryheld_over_held_required() EXCLUSIVE_LOCKS_REQUIRED(mu) { // expected-note {{mutex acquired here}}
    mu.TryLock(); // expected-note {{mutex acquired here}}
  } // expected-warning {{mutex 'mu' is still held at the end of function at an unexpected reentrancy depth}} \
    // expected-warning {{unchecked result of try-acquire; mutex 'mu' may still be held at the end of function}}
  ...
  // upgrades the try-held fact to a definite hold (addLock(),
  ...
  // the assert: any other release of a try-held fact is diagnosed at the
  ...
  // below is not diagnosed), and its escape still gets the beta leak
  // warning.
  ...
  // although 'b' is false again, so no resolution -- conservative try-held
  // behavior remains.
  ...
  // An address that is only readable through cannot mutate the variable,
  // so it is not an escape -- the same const distinction reference and
  // pointer parameters get. Both the conversion at the call and an
  // explicitly const-qualified pointer variable count.
  ...
  void tryheld_const_pointer_no_escape(bool c) {
    bool b = false;
    const bool *p = &b;
    if (c)
      b = mu.TryLock();
  ...
  // iteration's own result, which nothing around the loop checks. The
  // second call's fact is tracked beside the first's and reported
  // unchecked at the loop join (beta); the post-loop check sees only the
  // last result.
  ...
  // and the hold it witnesses is re-materialized on the success edge.
  ...
  // re-branch (the result is checked wherever it is used, so no leak
  ...
  // joining block's terminator re-branches on the merged result, the join
  ...
  // nothing about either; conservative try-held handling remains.
  ...
  // As above, but the spending release sits behind an unrelated condition:
  // the negative fact it leaves survives the joins as one-sided evidence
  // and refuses both the re-branch carry of the other path's try-held fact
  ...
  // loop head's join does not re-branch on the result, so the try-held
  ...
  // edge (no negative fact contradicts the stored result), so the success
  ...
  // definitely-held fact lost to the spending release is diagnosed with
  // the plain missing-on-some-paths warning at the re-branch instead.
  ...
  // A release that spends the stored result inside a loop body is not
  // forgotten: the spend evidence rides the back edge as a weak fact and
  // refuses re-resolving the post-loop branch to a live hold.
  ...
  // As above with the spend on a continue path: continue joins keep the
  // weak spend evidence like branch joins do.
  ...
  // A second try-acquire of the same mutex does not destroy the spend
  // evidence: the consumed spent negative survives as a weak fact, and any
  // spent negative for the capability vetoes carrying a try-held fact
  // whose result may be stale.
  ...
  // The spend evidence survives a continue-latch join that merges the
  // release's spent negative with the same call's failure-edge negative:
  // the merged fact keeps the spent marker for the post-loop resolution.
  ...
  // spend instead of blocking it from the loop's exit edges.
  ...
  // adopted by the re-branched call and resurrected on its failure edge.
  ...
  // A spend inside the loop reaches an exit edge owned by a block other
  // than the loop head (a break after the spend).
  ...
  // Two independent, properly checked try-acquires: the first one's spent
  ...
  // of the re-branch exemption too, not just the form whose fact carries
  ...
  // definite hold; `c && ok` re-branches on the call, but the c-false edge
  ...
  // diagnosed there (beta), and the later branch on the stored result does
  // NOT re-materialize the acquisition, because a negative fact for the
  // capability survives to veto it: the negative from the lock/unlock pair
  // is consumed at the call, but on the path that skips the call it flows
  // to the join and is kept as one-sided evidence -- the analysis cannot
  // tell it from a release that spent the result, so the success region
  // stays conservative, with the release as the note.
  ...
  // An unchecked release of a try-held fact leaves the capability provably
...
struct TestTrylockSwitch {
  ...
  // its block is analyzed only for coverage: the dead try-held state must
  // not flow into the switch exit as if it were a live path (previously a
...
struct TestTrylockValueCodes {
  ...
  // resolves as that does -- a truthy edge promotes both codes, and mu1's
  // result is never checked, so it leaks.
  ...
  bool try_both_polarities() EXCLUSIVE_TRYLOCK_FUNCTION(true, mu7)
      EXCLUSIVE_TRYLOCK_FUNCTION(false, mu7);
  void valuecodes_cond_resolves_nothing() {
    if (try_both_polarities() // expected-warning{{mutex 'mu7' is acquired regardless of the result of the try-acquire call; treating the acquisition as unconditional}}
            ? mu2.TryLock()
            : false) {
      data2 = 1;
      mu2.Unlock();
  ...
  // `== 1` and declare mu2 not acquired.
...
// A scoped relock between the spend and the re-branch preserves the spend
// evidence: the scoped acquisition consumes the spent negative only down to
// a weak fact, so the stale result still cannot resurrect the released
// hold.
...
// Re-branching on a try-acquire result re-adds the promoted fact unchanged,
// so a reentrant re-acquisition inside the guarded region keeps its depth
// across the re-branch and the unlocks stay balanced.
...
// An acquire over a try-held reentrant capability deepens the fact, keeping
// its origin: the capability is held at the deeper level iff the try-acquire
// succeeded, one level shallower otherwise, so branching on the result
// afterwards keeps the unlocks balanced for either outcome.
...
// while the depth imbalance is re-diagnosed at the later joins. The notes
// point at the definite acquisition that is unbalanced, not at the
// try-acquire.
void testReentrantTryLockConditionalDeepen(bool c) {
  bool b = rmu.TryLock();
  if (c)
    rmu.Lock(); // expected-note 4 {{mutex acquired here}}
