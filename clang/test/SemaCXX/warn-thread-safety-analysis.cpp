...
struct TestTryLock {
  ...
  // (try-held -> held), but is diagnosed: the capability may already be
  // held, since the try-acquire result was never checked. An assert upgrades
  // without a diagnostic -- it claims exactly that knowledge.
  ...
  // A try-held capability may be held: uses that require it to be free are
  ...
  // be tracked separately (one fact per capability; the first wins): the
  // second call itself does not change the state, and branching on its
  // result acquires over the first try-acquire, which may have succeeded.
  ...
  // A try-acquire of a capability that is provably held by an earlier
  // acquire: at runtime the call simply fails and the success branch is
  // dead, but the analysis cannot track the attempted acquisition (one
  // fact per capability), so the conflict is diagnosed at the call. The
  // dead branch's release then unbalances the live hold, which the
  // ordinary join diagnostics report.
  ...
  // As above without a release inside the dead branch: only the conflict
  // at the call is diagnosed; the hold is left unchanged on both edges, so
  // the join and the final release stay balanced.
  ...
  // Without a collision at the call there is no dead edge to model: the
  // success edge is live, and the merged hold is on that path the
  // try-acquire's own single acquisition, gaining no extra count. The
  // first join agrees (held once on both paths); the second join, after
  // the conditional release, mismatches and diagnoses -- this double
  // release is a genuine bug.
  void tryheld_merged_hold_double_release(bool c) {
    bool b = mu.TryLock();
    if (!b)
      mu.Lock(); // expected-note {{mutex acquired here}}
    if (b) {
  ...
  // A try-acquire of a capability whose hold is claimed by an assert: the
  // same conflict as over a real hold (the attempted acquisition goes
  // untracked), diagnosed at the call. Branching on the result leaves the
  // assert-claimed hold untouched on both edges, and the dead success
  // branch's release is absorbed by the asserted fact's join exemptions.
  ...
  // join is not re-materialized from the attribute arguments: uses of it in
  // the success region warn. Conservative -- the loss itself was already
  // diagnosed at the join (in beta mode) -- but a future refinement could
  // re-add capabilities whose call-time translation still matches.
  ...
  // A multi-capability try-acquire that collides on one capability: only
  // that capability goes untracked, and the conflict is diagnosed at the
  // call; the call's other capability stays tracked and resolvable through
  // the merged variable.
  void tryheld_multi_cap_partial_collision(bool c) {
    mu2.Lock();             // expected-note {{mutex acquired here}}
    bool ok = false;
    if (c)
      ok = TryLockBoth();   // expected-warning {{acquiring mutex 'mu2' that is already held}}
    if (ok) {
      a = 1;
      mu.Unlock();
    }
  ...
  // assert on one path: branching on the try-acquire result says nothing
  // about it, so the definite hold must not be silently discarded on the
  // failure edge -- the acquire over the merged (unresolvable) state and the
  // leak out of the function are diagnosed instead.
  ...
  // only one stored: the branch resolves the merge of the stored result
  // with the constant initializer, but the facts' merged origin is not
  // determined by either call, so it is cleared -- and both discarded
  // origins are diagnosed immediately at the join (the unchecked first
  // TryLock really can leak its acquisition). The success edge then
  // re-acquires the capability over the origin-cleared fact (the same
  // conservative residue as tryheld_failure_path_lock_rebranch), and the
  // failure edge cannot remove it, so it also leaks out of the function.
  void tryheld_merge_two_origins(bool c) {
    bool b = false;
    if (c)
      mu.TryLock();      // expected-note {{mutex acquired here}}
    else
      b = mu.TryLock();  // expected-note 3 {{mutex acquired here}} \
                         // expected-warning {{acquiring mutex 'mu' that may already be held}}
  ...
  // Re-branching on the result of a promoted fact is consistent with it; no
  // duplicate-acquire diagnostic.
  void tryheld_nested_rebranch() {
    bool b = mu.TryLock();
    if (b) {
      if (b) {
        a = 1;
  ...
  // beta warning at the join, and the branches on 'b' resolve through the
  // loop-merged variable, so the shape is warning-free end to end.
  ...
  // A spin loop storing the result: on loop exit the merged variable is
  // truthy, proving the try-acquire succeeded.
  ...
  // must absorb that rather than discard it.
  ...
  // Unlike the retry idiom, here the second call can execute while the
  // first call's capability is still tracked (its result was not checked),
  // so the second acquisition cannot be tracked and a successful first
  // try-acquire can leak whenever the loop iterates: the conflict is
  // diagnosed at the colliding call.
  void tryheld_retry_without_check() {
    bool b = mu.TryLock(); // expected-note {{mutex acquired here}}
    while (cond)
      b = mu.TryLock(); // expected-warning {{acquiring mutex 'mu' that may already be held}}
    if (b)
      mu.Unlock();
  }
  ...
  // Diagnosed at the loop join, where the unresolved fact is dropped.
  ...
  // re-branch (the result is checked wherever it is used, so nothing is
  // leaked).
  ...
  // Two DIFFERENT try-acquire calls stored on both paths of a branch: the
  // states agree (b true implies mu held) and the merge resolves to the
  // stored calls, but at the join the facts' merged origin is not
  // determined by either call and is cleared, so the branch cannot resolve
  // the fact: the success edge re-acquires over it and the failure edge
  // leaks it. Conservative; a future refinement could keep the origin of
  // the call the join's terminator re-branches on when the other origin is
  // the same call per the merge.
  void tryheld_merge_both_assigned(bool c) {
    ...
    if (c)
      ...
      b = mu.TryLock(); // expected-warning {{acquiring mutex 'mu' that may already be held}} \
                        // expected-note 3 {{mutex acquired here}}
    if (b)              // expected-warning 2 {{unchecked result of try-acquire; mutex 'mu' may still be held past this point}}
      mu.Unlock();
  } // expected-warning {{unchecked result of try-acquire; mutex 'mu' may still be held past this point}}
  ...
  // A merge of a try-acquire result with its constant initializer stays
  // resolvable across an intervening loop: the loop head wraps the merge in
  // reference definitions, and resolution looks through them to the phi.
  // The possible success carried into the loop head's join is still
  // diagnosed (beta), but the branch after the loop resolves the merge and
  // the release balances.
  ...
  // pair) is consumed at the call: it proves nothing about the call's
  // outcome and must not block re-adding the fact -- dropped at the
  // unrelated join -- when a branch finally tests the result.
...
// name a different capability (here: through a reassigned pointer) and must
// not promote or destroy what the call actually acquired.
...
// not just from a bare literal: a substituted template parameter (or any
// other constant expression) must keep the success edges' polarity -- a
// value that failed to decode would flip both edges.
...
// The success edge is where the result equals the (evaluated) success value;
// the other edge must not hold the capability.
void failure_edge() {
  if (try_lock_templ<false>()) {
...
// The guard's explicit unlock releases a capability whose try-acquire
// result was never checked: diagnosed like the direct call (removeLock()),
// and the fact is dropped without asserting the negative capability.
...
// The guard's definite acquire over a try-held capability keeps it
// try-held (a non-reentrant capability is a real double lock exactly when
// the try-acquire succeeded), and the write stays diagnosed.
...
// Unlike an exclusive capability (see tryheld_nested_try), a second
// try-acquire of a reentrant capability while it is try-held may
// legitimately succeed, so no may-already-be-held diagnostic is issued when
// branching on the second result.
...
// A deepen behind an unrelated branch joins try-held facts of unequal
// depths: the shallower fact survives the join -- keeping the deeper one
// would let the failure edge convert its below-top levels into a definite
// hold on paths that never acquired them, hiding the unguarded write.
...
// A deepen behind a branch on the result itself: the demotion at the
// re-branched join drops the depth (its below-top level is definite only on
// the deepened path), which is diagnosed right there as a reentrancy
// mismatch; the unguarded write on the failure path is then diagnosed too,
// and the depth going untracked across the join costs a conservative
// warning on the second release.
