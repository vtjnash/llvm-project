...
struct TestTryLock {
  ...
  // resolved, so the join's rebranch resolves the same call: mu's facts
  ...
  // through whole and every use stays conservatively diagnosed.
  void foo31() {
  ...
  // An unconditional acquire upgrades a conditionally held capability to held.
  ...
  // guarded release releases that result, and the reacquire is clean. The
  ...
  // loop-top check re-materialized meets the fresh call's conditional fact at
  ...
  // to conditional instead of diagnosing a mixed join.
  ...
  // result" and "the result is falsy" -- so the join is conditionally held,
  // silently again, and the release after the loop resolves cleanly.
  ...
  // value. The release marks the negative fact it leaves, the
  ...
  // the fall-through path's fresh conditional fact. Both facts originate
  // from the same call, so the latch join demotes silently to conditional
  ...
  // conditional fact while the result may be stale.
