...
///
/// A reentrant capability additionally tracks a reentrancy depth, and two
/// transitions deepen its fact instead of colliding with it (addLock()):
///
///   held -----try-acquire--------------------------------------> held,
///             one level deeper with the call as origin: a reentrant
///             try-acquire over a held capability always succeeds, so
///             the deeper hold is definite and a branch's failure edge
///             is infeasible
///   try-held --acquire-----------------------------------------> try-held,
///             one level deeper keeping the origin: held at this depth
///             iff the try-acquire succeeded, one level shallower
///             otherwise -- resolved exactly that way at a branch on the
///             result (getEdgeLockset()) and unwound one level per
///             release (removeLock())
///
...
/// conflict the model cannot represent (one fact per capability): the
/// existing fact wins unchanged, the call's acquisition goes untracked, and
/// the conflict is diagnosed at the call (addLock()) -- except over a
/// try-held reentrant capability, where the second acquisition may
/// legitimately succeed or fail with the first and is dropped silently;
/// that remaining case is NOT conservative and can under-report.
...
private:
  ...
  /// success, so a branch on its result must not resolve them (a reentrant
  /// acquire instead deepens the fact, keeping the origin for its still
  /// conditional top level). A try-held
...
public:
  ...
  /// conditional while the levels below it are definite (addLock()).
...
public:
  ...
  // try-held and negative facts do not count. Unlike isEmpty(), which tests
  // the set itself.
...
protected:
  ...
  // value is that of one of the two. A phi is opaque to lookupExpr(), so
  // most consumers treat it like a cleared definition; it exists so that a
  // branch on a try-acquire result merged with a constant initializer can
  // still be resolved (see getTrylockCallExpr()).
...
private:
  ...
  /// reentrantly (after initial acquisition). Nonzero only for reentrant
  /// capabilities.
...
public:
  ...
  // Return this fact with its reentrancy levels dropped. Used when demoting
  // to try-held across a join (intersectAndWarn()): a try-held fact asserts
  // its below-top levels as definite (definitelyHeld()), but the levels are
  // definite only on this fact's own paths, not across the join.
  const LockableFactEntry *withoutReentrancy(FactManager &FactMan) const {
    if (!ReentrancyDepth)
      return this;
    auto *NewFact = FactMan.createFact<LockableFactEntry>(*this);
...
/// The one policy for releasing a capability that is only try-held with no
/// definite levels below (reentrancy depth zero). TryHeld -> NotHeld is
/// only permitted when a branch tests the try-acquire result
/// (getEdgeLockset()), so this release may release a capability that is not
/// held: diagnose like an unmatched unlock and drop the fact. The fact is
/// deliberately not replaced by the negative fact (!Cp): this release
/// cannot be paired with the unchecked try-acquire, so it proves nothing
/// about whether the capability is held afterwards, and a later acquire
/// must still not claim the negative capability. With a null \p Handler (a
/// scoped guard's destructor) nothing is diagnosed or dropped: the
/// destructor releases only what it holds at runtime, like the double
/// releases it does not diagnose either -- but neither can it pair with the
/// unchecked try-acquire, so the fact stays, still carrying its obligation
/// to check the result. Returns true if the release was handled here.
static bool handleUncheckedTryHeldUnlock(FactSet &FSet, FactManager &FactMan,
                                         const FactEntry &Fact,
                                         const CapabilityExpr &Cp,
                                         SourceLocation UnlockLoc,
                                         ThreadSafetyHandler *Handler) {
  const auto *LFE = dyn_cast<LockableFactEntry>(&Fact);
  if (!Fact.tryHeld() || (LFE && LFE->getReentrancyDepth() != 0))
    return false;
  if (Handler) {
...
private:
  void lock(FactSet &FSet, FactManager &FactMan, const CapabilityExpr &Cp,
    ...
    if (const auto It = FSet.findLockIter(FactMan, Cp); It != FSet.end()) {
      ...
      // A try-held fact deliberately stays try-held under this definite
      // acquire: the new acquisition nests below the conditional top level,
      // so tryReenter() only deepens the reentrancy count, and the
      // try-acquire origin keeps its result-check and release-pairing
      // obligations. Upgrading TryHeld -> Held would silently discard both.
      // For a non-reentrant capability the double-lock diagnostic is also
      // intended: the definite acquire re-acquires the capability exactly
      // when the try-acquire succeeded.
  ...
  void unlock(FactSet &FSet, FactManager &FactMan, const CapabilityExpr &Cp,
    ...
    if (const auto It = FSet.findLockIter(FactMan, Cp); It != FSet.end()) {
      const auto &Fact = cast<LockableFactEntry>(FactMan[*It]);
      // Releasing a capability whose try-acquire result was never checked
      // (see handleUncheckedTryHeldUnlock() for the guard-destructor
      // nuances).
...
class ThreadSafetyAnalyzer {
  ...
  // For each try-acquire call, the blocks whose terminator branches on its
  // result (getTerminatorTrylockCall()), so a loop join can tell whether
  // the result is checked inside the loop (intersectAndWarn()).
  llvm::SmallDenseMap<const Expr *, llvm::SmallPtrSet<const CFGBlock *, 2>>
      TryResultBranches;
  // Maps constructed objects to `this` placeholder prior to initialization.
  llvm::SmallDenseMap<const Expr *, til::LiteralPtr *> ConstructedObjects;
  // Whether any fact originating from a try-acquire call has been created,
  // so that joins in functions without try-acquires (the common case) skip
  // scanning their fact sets for one.
  bool AnyTryLockFacts = false;
  FactManager FactMan;
  std::vector<CFGBlockInfo> BlockInfo;
...
public:
  ...
  /// decoded by getTerminatorTrylockCall(): the parts that depend only on
  /// the block, memoized in TerminatorTrylockCache.
  struct TrylockTerminator {
  ...
  /// TrylockTerminator plus the one per-call, non-memoizable part: the
  /// cleanup that keeps the SExprBuilder local-variable lookup closure
  /// installed (beta mode) so that the caller can translate the callee's
  /// attribute expressions.
  struct TerminatorTrylockCall : TrylockTerminator {
    std::optional<llvm::scope_exit<std::function<void()>>> Cleanup;
  };

  /// Per-block memo of the terminator decode, hits and misses both: a block
  /// is queried once per outgoing edge, plus once per join that walks the
  /// condition's short-circuit blocks (getConditionTrylockCallExpr()), and
  /// in beta mode each decode installs and tears down the SExprBuilder
  /// lookup closure. The decode depends only on the terminator and the
  /// variable map's exit context, both fixed by this phase.
  llvm::SmallDenseMap<unsigned, TrylockTerminator, 8> TerminatorTrylockCache;
...
/// \param CheckAcquisition -- false when the acquisition checks already ran
/// for this acquisition (a branch edge re-adding a fact dropped at a join,
/// see getEdgeLockset(): the checks run exactly once, at the call).
void ThreadSafetyAnalyzer::addLock(FactSet &FSet, const FactEntry *Entry,
                                   bool ReqAttr, bool CheckAcquisition) {
  ...
  if (const FactEntry *Cp = FSet.findLock(FactMan, *Entry)) {
    if (Entry->tryHeld()) {
      // Try-acquiring a capability that is already tracked. A reentrant
      // capability that is definitely held reenters: such a try-acquire
      // always succeeds at runtime, so the deeper hold is definite and
      // carries the call as its origin -- a later branch on the result
      // resolves it like any promoted fact, with an infeasible failure
      // edge (getEdgeLockset()).
      if (const auto *LCp = dyn_cast<LockableFactEntry>(Cp);
          LCp && !Cp->tryHeld())
        if (const FactEntry *RFact = LCp->tryReenter(FactMan, Entry->kind())) {
          FSet.replaceLock(FactMan, *Cp,
                           cloneWithTryLock(*RFact, Entry->tryLockCall(),
                                            /*Conditional=*/false));
          return;
        }
      // A reentrant capability that is only try-held: the second acquisition
      // may legitimately succeed or fail with the first, which one fact per
      // capability cannot represent; the first fact wins and the second
      // acquisition goes untracked (the documented under-reporting
      // simplification, see FactEntry).
      if (Cp->reentrant())
        return;
      // Otherwise the model cannot track this call's acquisition (one fact
      // per capability, the first wins): over a definite hold the call
      // provably fails and its success branch is dead; over a try-held fact
      // the capability's state stops being governed by either result alone.
      // Diagnose the conflict here, at the colliding call; the existing
      // fact is kept unchanged, and whatever the untracked acquisition's
      // dead branch unbalances downstream surfaces through the ordinary
      // join diagnostics.
    ...
    if (Cp->tryHeld()) {
      ...
      // unless it is an assert, which claims exactly that knowledge; the
      // capability is reentrant (deepened below); or this is a branch
      // edge's re-acquisition (getEdgeLockset()) over a fact that still
      // has an origin, whose collision was already diagnosed at the
      // colliding call above.
      if (!Entry->asserted() && !Entry->reentrant() &&
          !(Entry->tryLockCall() && Cp->tryLockCall()))
        Handler.handleDoubleLock(Entry->getKind(), Entry->toString(),
                                 Cp->loc(), Entry->loc(), /*MaybeHeld=*/true);
      // A reentrant acquire deepens the try-held fact instead of replacing
      // it, keeping its origin and conditional state: the capability is now
      // held at this depth if the try-acquire succeeded and one level
      // shallower otherwise -- resolved exactly that way at a branch on the
      // result (getEdgeLockset()) and unwound one level per release
      // (removeLock()).
      if (!Entry->asserted())
        if (const auto *LCp = dyn_cast<LockableFactEntry>(Cp))
          if (const FactEntry *RFact =
                  LCp->tryReenter(FactMan, Entry->kind())) {
...
/// not later testify that this call failed, see AddIfNotPromoted()); on the
...
const FactEntry *ThreadSafetyAnalyzer::cloneWithTryLock(const FactEntry &FE,
  ...
  // An origin can only be cloned from a fact addTryLock() created, which
  // already noted that the function has try-lock facts.
  assert(!Call || AnyTryLockFacts);
  return NewFact;
}
...
void ThreadSafetyAnalyzer::addTryLock(FactSet &FSet, const CapabilityExpr &CE,
  ...
  AnyTryLockFacts = true;
  addLock(FSet, Fact, /*ReqAttr=*/false, CheckAcquisition);
}
...
void ThreadSafetyAnalyzer::removeLock(FactSet &FSet, const CapabilityExpr &Cp,
  ...
  // Releasing a capability whose try-acquire result was never checked
  // (handleUncheckedTryHeldUnlock()). A try-held fact deepened by a
  // reentrant acquire (addLock()) is definitely held below its conditional
  // top level, so releasing it is matched: handleUnlock() below unwinds one
  // level, keeping the origin.
...
const CallExpr *
ThreadSafetyAnalyzer::getTrylockCallExpr(const Stmt *Cond, LocalVarContext C,
                                         bool &Negate,
                                         std::optional<bool> &AmbiguousCond) {
  if (!Cond)
    return nullptr;
  ...
  if (const auto *CallExp = dyn_cast<CallExpr>(Cond)) {
    if (CallExp->getBuiltinCallee() == Builtin::BI__builtin_expect)
      return getTrylockCallExpr(CallExp->getArg(0), C, Negate, AmbiguousCond);
    return CallExp;
  }
  else if (const auto *PE = dyn_cast<ParenExpr>(Cond))
    return getTrylockCallExpr(PE->getSubExpr(), C, Negate, AmbiguousCond);
  else if (const auto *CE = dyn_cast<ImplicitCastExpr>(Cond))
    return getTrylockCallExpr(CE->getSubExpr(), C, Negate, AmbiguousCond);
  else if (const auto *FE = dyn_cast<FullExpr>(Cond))
    return getTrylockCallExpr(FE->getSubExpr(), C, Negate, AmbiguousCond);
    ...
    if (const Expr *E = LocalVarMap.lookupExpr(DRE->getDecl(), DefCtx))
      return getTrylockCallExpr(E, DefCtx, Negate, AmbiguousCond);
    ...
    for (unsigned Op : {VDef->DirectRef, VDef->PhiAlt}) {
      ...
      if (getStaticBooleanValue(E, B, ACtx)) {
        if (K && *K != B)
          return nullptr; // Constants of both truthinesses determine nothing.
        K = B;
        ConstID = Op;
      } else if (NonConst) {
        NonConst2 = E;
    ...
    if (NonConst2) {
      ...
      return getTrylockCallExpr(NonConst, NonConstCtx, Negate, AmbiguousCond);
    }
    if (!NonConst || !K)
      return nullptr;
    ...
    AmbiguousCond = *K != Negate;
    return getTrylockCallExpr(NonConst, NonConstCtx, Negate, AmbiguousCond);
  }
  else if (const auto *UOP = dyn_cast<UnaryOperator>(Cond)) {
    if (UOP->getOpcode() == UO_LNot) {
      Negate = !Negate;
      return getTrylockCallExpr(UOP->getSubExpr(), C, Negate, AmbiguousCond);
    }
    return nullptr;
  }
    ...
    if (BOP->getOpcode() == BO_EQ || BOP->getOpcode() == BO_NE) {
      ...
      if (getStaticBooleanValue(BOP->getRHS(), TCond, *ASTCtx)) {
        ...
        return getTrylockCallExpr(BOP->getLHS(), C, Negate, AmbiguousCond);
      }
      TCond = false;
      if (getStaticBooleanValue(BOP->getLHS(), TCond, *ASTCtx)) {
        if (!TCond) Negate = !Negate;
        return getTrylockCallExpr(BOP->getRHS(), C, Negate, AmbiguousCond);
      }
      return nullptr;
    }
    ...
    if (getStaticBooleanValue(COP->getTrueExpr(), TCond, *ASTCtx) &&
      ...
      if (TCond && !FCond)
        return getTrylockCallExpr(COP->getCond(), C, Negate, AmbiguousCond);
      if (!TCond && FCond) {
        Negate = !Negate;
        return getTrylockCallExpr(COP->getCond(), C, Negate, AmbiguousCond);
      }
    ...
    if (K && !AmbiguousCond) {
      AmbiguousCond = *K != Negate;
      return getTrylockCallExpr(NonConstArm, C, Negate, AmbiguousCond);
    }
    ...
    if (const auto *CS = SE->getSubStmt(); CS && !CS->body_empty()) {
      if (const auto *E = dyn_cast<Expr>(CS->body_back()))
        return getTrylockCallExpr(E, C, Negate, AmbiguousCond);
    }
  }
  return nullptr;
}
...
ThreadSafetyAnalyzer::getTerminatorTrylockCall(const CFGBlock *Block) {
  ...
  // Temporarily set the lookup context for SExprBuilder (beta mode); the
  // scope-exit handed to the caller uninstalls it. Installed only when a
  // try-acquire call was identified: identifying it (getTrylockCallExpr())
  // consults the local-variable map alone, never SExprBuilder.
  auto InstallLookup = [&](TerminatorTrylockCall &Result) {
    if (!Handler.issueBetaWarnings())
      return;
    SxBuilder.setLookupLocalVarExpr(
        [this, Ctx = BlockInfo[BlockID].ExitContext](
            const NamedDecl *D) mutable -> const Expr * {
          return LocalVarMap.lookupExpr(D, Ctx);
        });
    Result.Cleanup.emplace([this] { SxBuilder.setLookupLocalVarExpr(nullptr); });
  };

  if (auto It = TerminatorTrylockCache.find(BlockID);
      It != TerminatorTrylockCache.end()) {
    TerminatorTrylockCall Result;
    static_cast<TrylockTerminator &>(Result) = It->second;
    if (Result.TrylockCall)
      InstallLookup(Result);
    return Result;
  }
  auto CacheMiss = [&] {
    TerminatorTrylockCache[BlockID] = TrylockTerminator{};
  ...
  bool Negate = false;
  std::optional<bool> AmbiguousCond;
  const auto *Exp = getTrylockCallExpr(Cond, BlockInfo[BlockID].ExitContext,
                                       Negate, AmbiguousCond);
  if (!Exp)
    return CacheMiss();
  ...
  // Remember where the call's result is branched on, so a loop join can
  // tell whether the result is checked inside the loop (intersectAndWarn()).
  TryResultBranches[Exp].insert(Block);

  TerminatorTrylockCall Result;
  static_cast<TrylockTerminator &>(Result) = {Exp, FunDecl, Negate,
                                              AmbiguousCond};
  TerminatorTrylockCache[BlockID] = Result;
  InstallLookup(Result);
  return Result;
}
...
/// it reports acquisition. Any expression that constant-evaluates is
/// recognized (a literal, but also e.g. a template parameter after
/// substitution); anything else defaults to false, as the branch decoding
/// always has.
...
bool ThreadSafetyAnalyzer::getEdgeLockset(FactSet &Result,
  ...
  const std::optional<bool> AmbiguousCond = Trylock.AmbiguousCond;
  ...
  // still a success edge. Translating the attribute arguments
  // (getMutexIDs()) is deferred until a consumer needs them
  // (TranslateAttrs() below): on most edges the facts recorded at the call
  // are resolved directly and a translation would be discarded -- and any
  // translation failure was already diagnosed at the call.
  ...
  // executed. Such an edge still resolves as a failure edge when even the
  // call executing would mean failure here (no attribute reports success on
  // this edge); otherwise leave the facts untouched, like an unresolved
  // condition. (A fact already promoted to held proves the call executed
  // and succeeded on every path into PredBlock -- the branch that promoted
  // it overwrote the constant -- so the infeasibility check below, which
  // implies the call would have failed here, remains correct.)
  if (AmbiguousCond && *AmbiguousCond == (CondVal == EdgeValue::True) &&
      AnySucceedsHere)
    return false;
  ...
  auto FactSucceedsHere = [&](const CapabilityExpr &FE) -> std::optional<bool> {
    if (!AnyFailsHere)
      return true;
    if (!AnySucceedsHere)
      return false;
  ...
  for (const auto &Fact : Result) {
    ...
    if (FE.negative()) {
      ...
      // the test is per fact, not per edge). Either way the call's result
      // is accounted for and its acquisition must not be re-added.
    ...
    if (*Succ) {
      ...
      Result.removeLock(FactMan, !*FE);
                   ...
                   isa<LockableFactEntry>(FE)
                       ...
                       : nullptr) {
      // The failure edge of a fact deepened by a reentrant acquire over
      // the try-held state (addLock()): the try-acquire's top level is not
      // held here, but the acquire's levels below it are -- definitely, so
      ...
      // implying success is infeasible, above) and does not resurrect an
      // acquisition this path has disproved or since released
      // (AddIfNotPromoted()).
      if (!Result.findLock(FactMan, !*FE)) {
  ...
  // dropped at an earlier join); anything not provably held goes through
  // addLock()'s usual acquisition checks. The facts keep their originating
  // call so that joins and later branches on its result can recognize them
  // (see intersectAndWarn()).
  TranslateAttrs();
  SourceLocation Loc = Exp->getExprLoc();
  auto AddIfNotPromoted = [&](const CapabilityExpr &CE, LockKind LK) {
    if (const FactEntry *Cp = Result.findLock(FactMan, CE)) {
      if (!Cp->tryHeld()) {
        // Already held: not this branch's transition to make. The
        // acquisition happens at the call (NotHeld -> TryHeld); a branch
        // only resolves it (TryHeld -> Held / NotHeld). A hold with this
        // call as its origin was resolved above; any other definite hold
        // either is (or subsumes) the call's own acquisition -- merged at
        // a join, the edge is simply consistent with it -- or the call
        // collided with it, which was diagnosed at the call (addLock())
        // and is not patched up here: whatever the untracked acquisition's
        // dead branch unbalances is reported by the ordinary join
        // diagnostics.
        return;
      }
    }
    if (Result.findLock(FactMan, !CE)) {
      // The capability is provably not held here -- the call failed, or its
      // acquisition was released since -- so the stored result no longer
      // witnesses a live acquisition: do not resurrect the fact.
      return;
    }
    // Re-adding a fact dropped at an earlier join is not a new acquisition:
    // the acquisition checks ran at the call and must not run (and diagnose)
    // again here.
...
void ThreadSafetyAnalyzer::warnIfMutexNotHeld(
  ...
  // the paths where the try-acquire succeeded. (One deepened by a reentrant
  // acquire is definitely held below its conditional top level, so it does.)
...
bool ThreadSafetyAnalyzer::join(const FactEntry &A, const FactEntry &B,
  ...
  if (ReentrancyDepthA != ReentrancyDepthB) {
    ...
    // For definitely-held facts, pick the FactEntry with the greater
    // reentrancy depth as the "good" fact to reduce potential later
    // warnings. A try-held pair keeps the shallower fact instead: the
    // deeper fact's levels below its conditional top are definite only on
    // its own paths, and the failure edge of a branch on the result would
    // convert them into a definite hold on paths that never acquired them
    // (getEdgeLockset()).
    bool TakeB = A.tryHeld() ? ReentrancyDepthB < ReentrancyDepthA
                             : ReentrancyDepthA < ReentrancyDepthB;
    return CanModify && TakeB;
...
void ThreadSafetyAnalyzer::intersectAndWarn(
  ...
  auto DemoteToTryHeld = [&, this](const FactEntry &FE,
    ...
    // Reentrancy levels below the top are definite only on this fact's own
    // paths; the fact is missing (or weaker) on the other side of the join,
    // so the demoted fact keeps none of them -- a try-held fact with depth
    // asserts them as definite everywhere (definitelyHeld()), and the
    // failure edge would unwind it into a definite hold (getEdgeLockset()).
    // Losing the levels is diagnosed here, at the join, not left to surface
    // as confusing follow-on warnings at the releases they had balanced.
    const FactEntry *Src = &FE;
    if (const auto *LFE = dyn_cast<LockableFactEntry>(&FE);
        LFE && LFE->getReentrancyDepth() != 0) {
      Handler.handleMutexHeldEndOfScope(FE.getKind(), FE.toString(), FE.loc(),
                                        JoinLoc, LEK,
                                        /*ReentrancyMismatch=*/true);
      Src = LFE->withoutReentrancy(FactMan);
    }
    return cloneWithTryLock(*Src, FE.tryLockCall(), /*Conditional=*/true);
  };
  // The one policy for diagnosing a fact the intersection removes (or
  // weakens to try-held), applied from either direction: a capability
  // managed by a scoped object is exempt -- the scoped fact still knows to
  // release it -- except where the scope itself ends or repeats.
    ...
    if (EntryIt != EntrySet.end()) {
      ...
      if (const FactEntry &Merged = FactMan[*EntryIt];
        ...
        if (EntryTryHeld && ExitFact.tryHeld()) {
          if (EntryOrigin)
            WarnNeverChecked(Merged, EntryFact.loc(), /*AtEndOfFunction=*/false);
          if (ExitFact.tryLockCall())
            WarnNeverChecked(Merged, ExitFact.loc(), /*AtEndOfFunction=*/false);
        }
      ...
      // diagnosed there. A negative fact from the call's failure edge is
      // simply not carried in: the re-branch re-derives it.
      if (!ExitFact.negative() && EntryLEK != LEK_LockedSomeLoopIterations) {
  ...
  for (const auto &Fact : EntrySetOrig) {
    ...
    if (!ExitFact) {
      if (IsTrylockRebranched(*EntryFact)) {
        ...
        // is left unmodified). A negative fact from the call's failure edge
        // holds on this path only and cannot be weakened; drop it -- the
        // re-branch re-derives it on the failure edge (getEdgeLockset()).
        if (EntryLEK != LEK_LockedSomeLoopIterations) {
          if (EntryFact->negative())
            EntrySet.removeLock(FactMan, *EntryFact);
          else if (!EntryFact->tryHeld()) {
  ...
  for (const auto *CurrBlock : *SortedGraph) {
    ...
    for (CFGBlock::const_succ_iterator SI = CurrBlock->succ_begin(),
      ...
      const Expr *RebranchTryLock =
          AnyTryLockFacts ? getConditionTrylockCallExpr(FirstLoopBlock)
                          : nullptr;
      ...
      if (Handler.issueBetaWarnings() && HasTryLockFact(LoopEnd->ExitSet)) {
        ...
        // blocks precede the latch in the traversal, so their branches have
        // been recorded by now; on an irreducible CFG the walk may escape
        // the loop, erring toward suppression.)
        ...
        for (const auto &[Call, Blocks] : TryResultBranches)
          if (llvm::any_of(Blocks, [&](const CFGBlock *B) {
                return LoopBlocks.count(B);
              }))
            CheckedInLoop.insert(Call);
      ...
      intersectAndWarn(PreLoop->EntrySet, LoopEnd->ExitSet, PreLoop->EntryLoc,
                       ...
                       /*RebranchResolvesAllPaths=*/true, CheckedInLoopPtr);
    }
  }
