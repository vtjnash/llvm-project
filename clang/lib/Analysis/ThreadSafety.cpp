...
public:
  ...
  /// Push every element of \p Other, discarding duplicates.
  void append_nodup(const CapExprSet &Other) {
    for (const CapabilityExpr &CapE : Other)
      push_back_nodup(CapE);
  }
};
...
private:
  ...
  /// lattice below: the join reads it, and no transition carries it, so two
  /// facts whose marks differ meet as an unmarked fact of the meet's state.
  bool ArmSplit : 1;
  ...
  TryFactEntry(const CapabilityExpr &CE, LockKind LK, SourceLocation Loc,
      ...
      : FactEntry(TryFact, CE, LK, Loc, Src), Origin(Origin),
        St(State::Conditional), ArmSplit(false), MayBeReleased(false) {
...
public:
  ...
  /// This try fact in state \p S, without marks: a ProvedNotHeld try fact's
  /// `?:` split mark and a Conditional try fact's MayBeReleased mark are kept
  /// only while the state stays. The fact itself if that is already its state.
  const TryFactEntry *withState(FactManager &FactMan, State S) const;
  /// This ProvedNotHeld try fact with its `?:` split mark set.
  const TryFactEntry *withArmSplit(FactManager &FactMan) const;
...
inline const TryFactEntry *TryFactEntry::withState(FactManager &FactMan,
                                                   State S) const {
  if (S == St && !ArmSplit && !MayBeReleased)
    return this;
  auto *NewFact = FactMan.createFact<TryFactEntry>(*this);
  NewFact->St = S;
  NewFact->ArmSplit = false;
  NewFact->MayBeReleased = false;
  return NewFact;
}

const TryFactEntry *TryFactEntry::withArmSplit(FactManager &FactMan) const {
  assert(provedNotHeld() && "only a failure edge splits a value merge");
  if (ArmSplit)
    return this;
  auto *NewFact = FactMan.createFact<TryFactEntry>(*this);
  NewFact->ArmSplit = true;
  return NewFact;
}
...
public:
  ...
  /// Remove every unresolved try fact of \p CapE that is *not* of \p Kind:
  /// none of them can be a level of a hold of that kind, while the ones
  /// that share it still resolve by the branch on their own result.
  void removeConditionalsOfOtherKind(FactManager &FM,
                                     const CapabilityExpr &CapE,
                                     LockKind Kind) {
    llvm::erase_if(FactIDs, [&](FactID ID) {
      if (!isConditionalOf(FM[ID], CapE) || FM[ID].kind() == Kind)
        return false;
...
/// cases: one of the other lock kind, which cannot be a level of this hold
/// at all (only those go -- a same-kind one beside them still resolves by
/// the branch on its own result, and dropping it took the report of its
/// own leak with it); and the acquiring scoped object's
...
static void addLockOverConditional(FactSet &FSet, FactManager &FactMan,
  ...
  if (!SameKind)
    FSet.removeConditionalsOfOtherKind(FactMan, *Entry, Entry->kind());
  else if (OwnOrigin)
...
class ThreadSafetyAnalyzer {
  ...
  struct TryAcquireCaps {
    ...
    /// A capability whose attribute keys the acquisition to a specific
    /// nonzero integer success code (a constant that is not a bool, e.g.
    /// TRY_ACQUIRE(2, mu)), with the kind that attribute promises it in:
    /// one entry per attribute per capability. Falsy values need no entry
    /// -- zero is the only falsy integer, so a falsy attribute's success
    /// region is exact already -- and a boolean success value promises
    /// acquisition on any nonzero result (TruthyAny).
    ///
    /// The kind is part of the key because a call can promise one
    /// capability in both kinds, on different outcomes: the try fact of
    /// each kind resolves by its own attributes' values, and reading
    /// across them would grant one kind's hold on the other kind's edge.
    struct ExactCode {
      CapabilityExpr Cap;
      LockKind Kind;
      llvm::APSInt Code;
    };
    llvm::SmallVector<ExactCode, 2> ExactCodes;
    ...
    /// compute), with the kind promising it: acquired on any nonzero
    /// result, so codes recorded for the same capability *in that kind*
    /// by other attributes do not bound its region.
    llvm::SmallVector<std::pair<CapabilityExpr, LockKind>, 2> TruthyAny;
    /// Capabilities reconcileTryAcquireCaps() moved out of the polarity
    /// groups: acquired in one kind regardless of the call's result.
    /// handleCall() turns them into unconditional acquisitions, with the
    /// diagnostic.
    ...
    /// Whether the call's attributes name any capability conditionally --
    /// the groups decodeTrylockBranch() builds its per-edge resolutions
    /// from. An unconditional acquisition resolves on no edge, so no try
    /// fact of one exists to re-resolve.
    bool hasConditionalCap() const {
      return !TruthyExclusive.empty() || !TruthyShared.empty() ||
             !FalsyExclusive.empty() || !FalsyShared.empty();
    }
...
public:
  ...
  struct TrylockDecode {
    ...
    bool Negate = false;
    /// The branch-condition truthiness of the edges where the branched-on
    /// value may be a merge's constant rather than the call's result --
    /// where the call may not have executed at all, so a resolution holds
    /// only if it did (see decodeTrylockCond()). Both directions can be
    /// marked, by two merges that make opposite ones ambiguous.
    bool AmbiguousTrue = false, AmbiguousFalse = false;
    /// The branch-condition truthiness of the edges that determine nothing
    /// about the result. Unlike the ambiguous edges the call did execute
    /// on these -- a `?:` merge identifies its result on one truthiness
    /// only -- so the edge must resolve no fact at all.
    bool UnknownTrue = false, UnknownFalse = false;

    bool ambiguousOn(bool Cond) const {
      return Cond ? AmbiguousTrue : AmbiguousFalse;
    }
    bool anyAmbiguous() const { return AmbiguousTrue || AmbiguousFalse; }
    bool unknownOn(bool Cond) const {
      return Cond ? UnknownTrue : UnknownFalse;
    }
    bool anyUnknown() const { return UnknownTrue || UnknownFalse; }
    /// Record that the edges of branch-condition truthiness \p Cond may
    /// carry a merge's constant. Where a merge has already marked the
    /// other direction, the two together identify the result on no edge
    /// at all: the branch still names the call, so a join keeps its try
    /// fact and a later branch can still resolve it, but every direction
    /// is left undetermined.
    void markAmbiguous(bool Cond) {
      if (ambiguousOn(!Cond))
        UnknownTrue = UnknownFalse = true;
      (Cond ? AmbiguousTrue : AmbiguousFalse) = true;
    }
    void markUnknown(bool Cond) { (Cond ? UnknownTrue : UnknownFalse) = true; }
    ...
    /// The destination type of the innermost of those conversions -- the
    /// type the value is truncated to, every conversion above it keeping
    /// the value. A constant a merge carries reaches the comparison
    /// converted to it, so that is where constantMeetsCond() has to read
    /// it, and a compared value the type cannot represent is never equal
    /// to the value at all. Null while ValueNarrowed is set means the
    /// walk merged two paths that narrowed differently, where no such
    ...
    QualType CmpType;
    /// The branched-on value carries the result's truthiness but not its
    /// magnitude: a `?:` merge of two constants selected by the result,
    /// the boolean a `&&`/`||` or a folded comparison produces from it.
    /// An exact value an edge carries (a case label) must not be applied
    /// to the result (clears TrylockBranch::ValueIsResult). Unlike
    /// ValueNarrowed, which records a conversion of the result itself,
    /// nothing below the walk resets this.
    ...
    /// (escapeReaches()), and which decides whether a logical operator's
    /// right-hand side is evaluated in situ.
...
/// Whether \p Call's try facts of the two lock kinds for \p CE are
/// companions: the call promises each kind on results the other does not,
/// so the branch that proves one refutes the other and the two holds never
/// coexist. Two attributes whose success regions overlap -- both promising
/// the capability on a falsy result, or on truthy results they share -- are
/// not that: one result cannot promise both an exclusive and a shared hold,
/// and the second acquisition is refused at the call.
bool ThreadSafetyAnalyzer::crossKindCompanion(const Expr *Call,
                                              const CapabilityExpr &CE) {
  auto It = TryAcquireCapsMap.find(Call);
  if (It == TryAcquireCapsMap.end())
    return false;
  const CapProfile Excl = getCapProfile(It->second, CE, LK_Exclusive);
  const CapProfile Shared = getCapProfile(It->second, CE, LK_Shared);
  if (!(Excl.Truthy || Excl.Falsy) || !(Shared.Truthy || Shared.Falsy))
    return false; // Only one kind promises it at all.
  if (Excl.Falsy && Shared.Falsy)
    return false; // Both on a zero result.
  if (!Excl.Truthy || !Shared.Truthy)
    return true;
  if (Excl.AnyNonzero || Shared.AnyNonzero)
    return false; // One of them takes every nonzero result.
  return llvm::none_of(Excl.Codes, [&](const llvm::APSInt &C) {
    return llvm::any_of(Shared.Codes, [&](const llvm::APSInt &S) {
      return llvm::APSInt::isSameValue(C, S);
    });
  });
}
...
bool ThreadSafetyAnalyzer::callResolvesConditionally(const Expr *Call) {
  auto It = TryAcquireCapsMap.find(Call);
  return It != TryAcquireCapsMap.end() && It->second.hasConditionalCap();
}
...
/// this acquisition can neither nest in nor coexist with -- unless it is
/// this same call's try fact of the other kind, its companion (below) --
/// and a repeat of this call over its own unresolved try fact (one try
/// fact per call and kind). A hold of a scoped object is not such a case:
/// the call is one of the guard's own members, and speaks about what the
/// guard manages.
...
bool ThreadSafetyAnalyzer::addTryLock(FactSet &FSet, const CapabilityExpr &CE,
  ...
  // call's try fact of the other kind (its companion, below) was checked
  // already.
  if (!FSet.findTryFactFrom(FactMan, CE, Call,
                            TryFactEntry::State::Conditional))
  ...
  for (const TryFactEntry *W : TryFacts) {
    if (W->conditional()) {
      // The same call's try fact of the other kind is its companion where
      // the call promises each kind on a *different* outcome: the edges
      // then resolve the two try facts by their own attributes, on edges
      // that exclude each other. Two attributes promising both kinds on
      // one outcome promise a hold this analysis cannot represent, and
      // fall through to the report below.
      if (W->origin() == Call && W->kind() != LK &&
          crossKindCompanion(Call, CE))
        continue;
...
// Whether \p V is a value of type \p Ty: converting it there and back
// keeps it. A comparison against a value the operand's type cannot hold is
// never true.
static bool representableIn(const ASTContext &Ctx, const llvm::APSInt &V,
                            QualType Ty) {
  llvm::APSInt Conv = V.extOrTrunc(Ctx.getIntWidth(Ty));
  Conv.setIsSigned(Ty->isSignedIntegerOrEnumerationType());
  return llvm::APSInt::isSameValue(Conv, V);
}

// The truthiness of a constant merge arm as the branch reads it. A
// conversion the walk looked through changes it only where it can make a
// nonzero value falsy (TruthinessLost: `char c = ok ? 256 : 0` stores zero
// whichever arm ran); anything else -- a conversion to bool above all --
// leaves the arm's own truthiness the answer. False where the arm is not a
// constant, or where a truncating conversion's own type is not known.
static bool getMergedArmTruthiness(const Expr *E, bool &Cond,
                                   const ThreadSafetyAnalyzer::TrylockDecode &D,
                                   const ASTContext &Ctx) {
  if (!D.ValueNarrowed || !D.TruthinessLost)
    return getStaticBooleanValue(E, Cond, Ctx);
  if (D.NarrowedTo.isNull() || !D.NarrowedTo->isIntegralOrEnumerationType())
    return false;
  Expr::EvalResult ER;
  if (E->isValueDependent() || !E->EvaluateAsInt(ER, Ctx))
    return false;
  Cond = ER.Val.getInt().extOrTrunc(Ctx.getIntWidth(D.NarrowedTo)) != 0;
  return true;
}

/// Whether \p B evaluates the innermost right-hand side of the logical
/// operator \p BOP (the RHS itself, or the RHS of the rightmost of a chain
/// of logical operators): the CFG threads a condition's logical operators
/// as terminators, so the block that evaluates the innermost RHS is the
/// one branching on the whole condition in situ, while an operator whose
/// value is materialized (under a negation, into a variable) appears as an
/// element of a value join instead.
static bool blockEvaluatesInnermostRHS(const CFGBlock *B,
                                       const BinaryOperator *BOP) {
  if (!B)
    return false;
  const Expr *E = BOP->getRHS();
  while (const auto *Inner = dyn_cast<BinaryOperator>(E->IgnoreParens())) {
    if (!Inner->isLogicalOp())
      break;
    E = Inner->getRHS();
  }
  E = E->IgnoreParenImpCasts();
  return llvm::any_of(*B, [&](const CFGElement &El) {
    std::optional<CFGStmt> CS = El.getAs<CFGStmt>();
    return CS && CS->getStmt() == E;
  });
}
...
// value -- as the comparison reads it, which is the constant converted
// through the truncation the walk looked through (TrylockDecode::NarrowedTo:
// `short s = ok ? 65537 : 0` compares s against 1, not 65537). Returns
// nullopt when a compared constant does not evaluate as an integer, or
// where the truncation it went through is not a single known type, and the
// caller can conclude nothing about either edge. The result is the
// condition's truthiness before \c Negate is applied.
...
constantMeetsCond(const Expr *ConstE, bool K,
  ...
  if (!ConstE || ConstE->isValueDependent() || !ConstE->EvaluateAsInt(ER, Ctx))
    return std::nullopt;
  llvm::APSInt Const = ER.Val.getInt();
  if (D.ValueNarrowed) {
    if (D.NarrowedTo.isNull() || !D.NarrowedTo->isIntegralOrEnumerationType())
      return std::nullopt;
    Const = Const.extOrTrunc(Ctx.getIntWidth(D.NarrowedTo));
    Const.setIsSigned(D.NarrowedTo->isSignedIntegerOrEnumerationType());
  }
  return llvm::APSInt::isSameValue(Const, *D.CmpValue);
}
...
void ThreadSafetyAnalyzer::decodeTrylockCond(const Stmt *Cond,
                                             LocalVarContext C,
                                             TrylockDecode &D) {
  // Continue the decode into one operand of a value merge, under the marks
  // \p Probe carries for the edges that operand cannot account for, and
  // adopt the result only if it found a call this branch can resolve.
  // Whichever operand answers first is the one the branch names: a merge
  // carries one call's result, not two.
  auto ProbeOperand = [&](const Expr *E, TrylockDecode Probe,
                          bool MustResolve) {
    decodeTrylockCond(E, C, Probe);
    if (!Probe.TrylockCall ||
        (MustResolve && !callResolvesConditionally(Probe.TrylockCall)))
      return false;
    D = Probe;
    return true;
  };
  ...
  if (const auto *CallExp = dyn_cast<CallExpr>(Cond)) {
    ...
    // result. Record (markAmbiguous()) the branch-condition truthiness of
    ...
    if (NonConst2) {
      ...
      if (!Second || D2.MergedCall || D2.Negate != D.Negate ||
          D2.AmbiguousTrue != D.AmbiguousTrue ||
          D2.AmbiguousFalse != D.AmbiguousFalse ||
          D2.UnknownTrue != D.UnknownTrue ||
          D2.UnknownFalse != D.UnknownFalse ||
          D2.ValueMerged != D.ValueMerged ||
      ...
      // decided by whichever was walked first. Paths that truncate to
      // different types (or only one of which truncates) leave no single
      // type the merged value is a copy in, which NarrowedTo records as
      // null.
      if (D2.ValueNarrowed != D.ValueNarrowed || D2.NarrowedTo != D.NarrowedTo)
        D.NarrowedTo = QualType();
    ...
    D.markAmbiguous(*ConstMeetsCond != D.Negate);
    ...
    if (BOP->getOpcode() == BO_EQ || BOP->getOpcode() == BO_NE) {
      ...
      // A comparison the walk folds to a truthiness test (`r != 0`, and
      // `r == 0` inverted) leaves the branch reading the comparison's own
      // boolean, whose magnitude is 0 or 1 whatever the result was: a case
      // label on it pins no code (`int v = r != 0; switch (v) { case 1: }`
      // is every nonzero result, not the code 1).
      D.ValueMerged = true;
      D.Negate = !D.Negate;
      return decodeTrylockCond(VarSide, C, D);
    }
    if (BOP->getOpcode() == BO_LAnd || BOP->getOpcode() == BO_LOr) {
      const bool K = BOP->getOpcode() == BO_LOr;
      // A left-hand side the constant evaluator decides leaves no
      // short-circuit edge and no value join: either it decides the
      // operator, and the right-hand side never runs at all, or it does
      // not, and the value is the right-hand side's own -- in situ
      // wherever the operator sits. `#define CONFIG_X 0` and a `constexpr
      // bool kSingleThreaded = false` are the shapes that reach this;
      // reading them as a merge made the branch ambiguous on an edge the
      // CFG does not have.
      bool LHSCond;
      if (getStaticBooleanValue(BOP->getLHS(), LHSCond, *ASTCtx)) {
        if (LHSCond == K || !D.foldCompareToBoolean())
          return;
        return decodeTrylockCond(BOP->getRHS(), C, D);
      }
      // In situ -- the block's own terminator condition, whose right-hand
      // side (the innermost of a chain of logical operators) is evaluated
      // in this very block, reached only on the non-short-circuiting edge
      // (the LHS was tested in an earlier block) -- the branch here is
      // exactly a branch on the RHS.
      if (blockEvaluatesInnermostRHS(D.UseBlock, BOP)) {
        ...
        // success code. (`foldCompareToBoolean()` drops the comparison for
        // the one value the operator can produce and refuses every other.)
        if (!D.foldCompareToBoolean())
          return;
        // The short-circuit path may have left the LHS's block without
        // evaluating the RHS at all (see TrylockDecode::ShortCircuit).
        D.ShortCircuit = true;
        return decodeTrylockCond(BOP->getRHS(), C, D);
      }
      // Otherwise the operator's value was materialized at a join the
      // short-circuit edge also reaches: `!(c || mu.TryLock())` puts the
      // operator under the negation, and a stored variable or a comparison
      // sits at such a join likewise. On the short-circuit edge the value
      // is the LHS's constant -- true for `||`, false for `&&` -- and the
      // RHS never ran; on the other it is a boolean copy of the RHS's
      // value. That is a merge of the result with a constant: the edges
      // where the value may be the constant are ambiguous (the call may
      // not have executed), the other edge proves the call ran and what
      // it returned. One such merge per condition, as for a phi; a value
      // comparison above compares the operator's own boolean.
      if (!D.foldCompareToBoolean())
        return;
      D.markAmbiguous(K != D.Negate);
      D.ValueMerged = true;
      return decodeTrylockCond(BOP->getRHS(), C, D);
    }
    ...
    bool TCond, FCond;
    if (getMergedArmTruthiness(COP->getTrueExpr(), TCond, D, *ASTCtx) &&
        getMergedArmTruthiness(COP->getFalseExpr(), FCond, D, *ASTCtx)) {
      // Both arms are constants: the value the condition selects is one of
      // them, so its magnitude is the arm's and proves nothing about the
      // result's -- an exact value a later edge carries (a case label)
      // must not be applied to the result (ValueMerged clears
      // ValueIsResult). What the branch asks of that value is what decides
      // the condition. A comparison against a specific value is satisfied
      // exactly when the condition selected the arm holding it, whatever
      // the two arms' truthiness (`(t ? 1 : 2) == 1` is `t` as much as
      // `(t ? 1 : 0) == 1` is), and a value neither arm holds is never
      // equal, so it determines nothing. A plain branch instead reads the
      // value's truthiness, which is the condition's only where the arms
      // differ in it.
      if (D.CmpValue) {
      ...
      // Both arms were read through the conversion above, so the branch is
      // now exactly a branch on the condition: what the conversion does to
      // the merged value is settled, and only a conversion below this one
      // still stands between the condition and the call.
      D.ValueNarrowed = false;
      D.TruthinessLost = false;
      D.NarrowedTo = QualType();
      D.ValueMerged = true;
    ...
    // The non-constant arm may hold the call itself
    // (`c ? mu.TryLock() : false`): an edge whose value-truthiness is !K
    // proves that arm ran and carries its result, while an edge matching K
    // may carry the constant with the call never executed, which is what
    // the ambiguous mark records. Only one such merge can be resolved per
    // condition.
    //
    // The arm takes priority over the condition below. The `?:` terminator
    // has already honored the condition's own branch
    // (decodeTrylockBranch()), so its result is resolved and its arm join
    // reconstituted; the arm's result has been branched on nowhere else,
    // and this is the only branch that can resolve it. It is also what the
    // `if` spelling of the same merge does, whose value identifies the arm
    // (`if (X) v = Y(); else v = false; if (v)` resolves Y).
    {
      ...
      ArmD.markAmbiguous(*ConstMeetsCond != D.Negate);
      if (ProbeOperand(NonConstArm, ArmD, /*MustResolve=*/false))
        return;
    }
    // Otherwise the call may sit in the condition
    // (`mu.TryLock() ? other() : 0`): an edge whose value-truthiness is !K
    // proves the non-constant arm ran, i.e. the condition selected it --
    // the condition's truthiness there is whether that arm is the true arm
    // -- while an edge matching K determines nothing: the value may be the
    // constant (the condition selected the other arm), or the arm's result
    // happening to share the constant's truthiness (with the condition --
    // the call's result -- unrestricted). Record those directions as
    // undetermined (markUnknown()); a comparison of the merged value
    // against a specific constant (CmpValue) identifies neither arm, so it
    // is refused.
    if (!D.anyUnknown() && !D.CmpValue) {
      TrylockDecode CondD = D;
      CondD.markUnknown(*K != D.Negate);
      ...
      ProbeOperand(COP->getCond(), CondD, /*MustResolve=*/true);
    }
    return;
    ...
    // nothing). With x non-constant, x may hold the call itself
    // (`flag ?: mu.TryLock()`) -- a falsy value proves both operands
    // falsy, and a truthy one may be r without the call executing, which
    // is the ambiguous edge's premise. It takes priority over r for the reason
    // the arm does above. Failing that, the call sits in r, where a truthy
    // value no longer determines its result (x may have produced it):
    // those edges are recorded undetermined as for the one-constant-arm
    // `?:` above.
    bool FCond;
    ...
    {
      TrylockDecode ArmD = D;
      ArmD.markAmbiguous(!D.Negate);
      if (ProbeOperand(BCO->getFalseExpr(), ArmD, /*MustResolve=*/false))
        return;
    }
    if (!D.anyUnknown() && !D.CmpValue) {
      TrylockDecode CondD = D;
      CondD.markUnknown(!D.Negate);
      ProbeOperand(BCO->getCommon(), CondD, /*MustResolve=*/true);
    }
    return;
...
ThreadSafetyAnalyzer::getConditionTrylockCallExpr(const CFGBlock *Block,
  ...
  if (CheckAllPaths) {
    ...
    // Finding the branch is not enough: it has to decide some capability
    // on some edge. A switch on the result whose labels pin nothing
    // (`switch (ok) { default: }`) leaves every edge Unknown, so
    // getEdgeLockset() resolves nothing and a silently demoted hold would
    // leak unreported. Naming the call is not that test: an edge carries
    // the call whenever it pins the branched-on value's truthiness, while
    // what it proves per capability is the resolution -- which a `?:`
    // merge's undetermined direction, or a code-keyed capability under a
    // truthiness edge, leaves Unknown on every direction of a branch that
    // still names the call. A branch that resolves some of its edges keeps
    // the promise where it can; what escapes the others is the ordinary
    ...
    for (const CFGBlock *D : Deciders) {
      ...
      for (CFGBlock::const_succ_iterator SI = D->succ_begin(),
        ...
        if (const CFGBlock *Succ = SI->getReachableBlock()) {
          ...
          AnyEdgeResolves =
              Edge.Infeasible ||
              llvm::any_of(Edge.Caps, [](const TrylockEdgeCap &EC) {
                return EC.Resolution != CapResolution::Unknown;
              });
        }
      if (!AnyEdgeResolves)
        Result.ResolvesAllPaths = false;
...
CapProfile ThreadSafetyAnalyzer::getCapProfile(const TryAcquireCaps &Caps,
  ...
  // Everything the attributes promising the capability in \p Kind say
  // about it, and nothing the other kind's say: the try fact of each kind
  // resolves by its own attributes, so a call promising an exclusive hold
  // on one outcome and a shared one on another resolves each try fact on
  // its own edge. Reading across the kinds grants one kind's hold where
  // only the other was promised.
  P.Truthy =
  ...
  for (const auto &Recorded : Caps.ExactCodes)
    if (Recorded.Kind == Kind && Probe.matches(Recorded.Cap))
      P.Codes.push_back(Recorded.Code);
  P.AnyNonzero =
      P.Codes.empty() || llvm::any_of(Caps.TruthyAny, [&](const auto &R) {
        return R.second == Kind && Probe.matches(R.first);
      });
  return P;
}
...
ThreadSafetyAnalyzer::decodeTrylockBranch(const CFGBlock *Block) {
  ...
  // other narrowing conversion (`short q = r;`) is a truthiness test of
  // the converted copy on one edge only: where the comparison holds, the
  // copy is nonzero and so is the result; where it does not, the result
  // may be any other code -- unlike a boolean copy, whose `!= 1` is
  // `== 0`. That edge resolves nothing: see TrylockDecode::TruthinessLost
  // and the narrowed comparison below.
  ...
  if (D.CmpValue) {
    ...
    if (BooleanCompared) {
      ...
      // A value the copy's own type cannot represent is one no copy ever
      // equals, so the comparison is never true: the edge it holds on is
      // unreachable and the other one is taken whatever the result was.
      // Neither says anything -- reading the equal edge as truthy below
      // would manufacture a hold on the unreachable one and carry it to
      // the join (`short q = f(); if (q == 65536)`).
      if (!D.NarrowedTo.isNull() &&
          D.NarrowedTo->isIntegralOrEnumerationType() &&
          !representableIn(*ASTCtx, *D.CmpValue, D.NarrowedTo))
        return CacheMiss();
  ...
  // groups) creates no try facts, so a branch on its result proves
  // nothing and records nothing here.
  if (MapIt == TryAcquireCapsMap.end() || !MapIt->second.hasConditionalCap())
    return CacheMiss();
  ...
  if (!ValueKeyed)
    D.CmpValue.reset();
  Result.AmbiguousTrue = D.AmbiguousTrue;
  Result.AmbiguousFalse = D.AmbiguousFalse;
  ...
  // (`switch (c && f())`, whose labels name that boolean), nor a value a
  // `?:` merge carries with the arm's own magnitude (ValueMerged) or
  // leaves undetermined on one direction (TrylockDecode::UnknownTrue).
  Result.ValueIsResult = ValueKeyed && !D.Negate && !D.CmpValue &&
                         !D.ValueNarrowed && !D.ShortCircuit &&
                         !D.anyUnknown() && !D.ValueMerged;
  ...
  //    capability keyed to codes, or recorded under both polarities in
  //    this kind (a result domain they do not cover, kept conditional),
  //    resolves only where an exact value does.
  // A direction recorded as undetermined by the decode (a `?:` merge, see
  // TrylockDecode::UnknownTrue) resolves nothing for any capability.
  {
    auto AddCaps = [&](const CapExprSet &CapSet, LockKind LK) {
      for (const CapabilityExpr &CE : CapSet) {
        CapProfile P = getCapProfile(Caps, CE, LK);
        ...
        if (D.UnknownTrue)
          Result.OnTrue.back().Resolution = CapResolution::Unknown;
        if (D.UnknownFalse)
          Result.OnFalse.back().Resolution = CapResolution::Unknown;
  ...
  assert((!Result.OnTrue.empty() || !Result.OnFalse.empty()) &&
         "a conditional capability records a resolution on both directions");
...
bool ThreadSafetyAnalyzer::getEdgeLockset(FactSet &Result,
  ...
  auto ResolveFact = [&](const CapabilityExpr &FE, LockKind Kind) {
    // The record has one entry per capability and kind, and every try fact
    // was created from that record, so a fact resolves by the entry of its
    // own kind -- a cross-kind call's two try facts resolve on different
    // edges -- and a fact of a kind the record does not name cannot exist.
    // Resolving one by the other kind's entry would read the opposite
    // conclusion.
    ...
    assert(llvm::none_of(
               Edge.Caps,
               [&](const TrylockEdgeCap &C) { return FE.matches(C.Cap); }) &&
           "a try fact of a kind its call's record does not name");
  ...
  for (const auto &Fact : Result) {
    ...
    case TryFactEntry::State::ProvedHeld:
      ...
      if (!Ambiguous && ResolveFact(*W, W->kind()) == CapResolution::Failure)
        Infeasible = true;
      break;
    case TryFactEntry::State::ProvedNotHeld:
      ...
      // executed.
      if (!Ambiguous && ResolveFact(*W, W->kind()) == CapResolution::Success)
        Infeasible = true;
      break;
    }
  }
  ...
  for (const TryFactEntry *W : Resolvable) {
    const CapResolution R = ResolveFact(*W, W->kind());
    ...
    if (Succeeds) {
      ...
      const TryFactEntry *NotHeldW =
          W->withState(FactMan, TryFactEntry::State::ProvedNotHeld);
      // A value `?:` split its arms to make this edge: mark the failure
      // so their join reconstitutes silently even without beta.
      if (const auto *COp = dyn_cast_if_present<AbstractConditionalOperator>(
              PredBlock->getTerminatorStmt());
          COp && !COp->getType()->isVoidType())
        NotHeldW = NotHeldW->withArmSplit(FactMan);
      Result.replaceFact(FactMan, *W, NotHeldW);
...
void BuildLockset::handleCall(const Expr *Exp, const NamedDecl *D,
  ...
  if (Exp && TryCaps) {
    ...
    // plus falsy: a result domain the two do not cover, which the
    // reconciliation above keeps conditional -- a boolean one it moves to
    // the unconditional groups, with a diagnostic) tracks one try fact per
    // kind it is promised in: a cross-kind pairing promises an exclusive
    // hold on one outcome and a shared one on another, and each try fact
    // resolves on its own attribute's edge (getEdgeLockset()).
    //
    ...
    // refused with "may already be held" at the call, since the two are not
    // companions (addTryLock(), crossKindCompanion()).
    //
    ...
    //
    // The definite lookups below are linear, so the question that cannot
    // have an answer is not asked: a call that acquires nothing definitely
    // has no definite list to consult.
...
private:
  ...
  const TryFactEntry *sameOriginProof(const FactSet &OwnSet,
                                      ...
                                      const FactEntry &FE) const;
  bool sameOriginGate(LockErrorKind LEK, const TryFactEntry *NotHeldW) const;
  /// The ProvedNotHeld try fact of \p Cap from \p Call in \p Set, if any.
  const TryFactEntry *failureRecord(const FactSet &Set,
                                    const CapabilityExpr &Cap,
  ...
  std::optional<Demotion> holdDemotion(const FactSet &OwnSet,
    ...
    if (Proof || isRebranchedInverseHold(OtherSet, FE)) {
      ...
      // proof of its own and takes the rebranched call. A rebranch that
      // does not resolve the result on every outgoing path still demotes,
      // and is reported there (joinDefiniteFromExit()): a branch that names
      // the call is the one that should have resolved it, so a same-origin
      // proof beside it does not make the silence right.
    ...
    if (!W || !sameOriginGate(LEK, failureRecord(OtherSet, FE, W->origin())))
      return std::nullopt;
  ...
  /// Whether \p EntryFact and \p ExitFact are the two kinds one cross-kind
  /// try-acquire promises, each proved on its own side by that call's try
  /// fact of that kind. The two are outcomes of one call, on edges that
  /// exclude each other, not an exclusive and a shared acquisition made in
  /// one scope.
  bool crossKindPairOfOneCall(const FactEntry &EntryFact,
                              const FactEntry &ExitFact) const {
    if (EntryFact.negative() || EntryFact.kind() == ExitFact.kind())
      return false;
    const TryFactEntry *E = EntrySetOrig.findTryFactIf(
        FactMan, EntryFact, [&](const TryFactEntry &W) {
          return W.provedHeld() && W.kind() == EntryFact.kind();
        });
    if (!E)
      return false;
    const TryFactEntry *X =
        ExitSet.findTryFactIf(FactMan, ExitFact, [&](const TryFactEntry &W) {
          return W.provedHeld() && W.kind() == ExitFact.kind() &&
                 W.origin() == E->origin();
        });
    return X && Analyzer.crossKindCompanion(E->origin(), EntryFact);
  }
...
// always; at branch joins where the hidden leak stays diagnosable or was
// never diagnosed -- under -Wthread-safety-beta (the unchecked-result
// diagnostics report it downstream) or for a `?:` arm split (\p NotHeldW
// carries the mark: the branch was never honored before, so its join
// never warned). Otherwise the eager lost-hold diagnosis at the join is
// the only coverage and is retained. Naming both join kinds also keeps
// the exemption away from the end-of-function comparison, whose entry set
// is the declared expected set: rewriting that would swallow the
// still-held diagnostic. (No try fact there today, so this only pins the
// invariant.)
bool LocksetJoin::sameOriginGate(LockErrorKind LEK,
                                 const TryFactEntry *NotHeldW) const {
  return LEK == LEK_LockedSomeLoopIterations ||
         (LEK == LEK_LockedSomePredecessors &&
          (Handler.issueBetaWarnings() || (NotHeldW && NotHeldW->armSplit())));
}
...
void LocksetJoin::joinTryFactPair(FactSet::iterator EntryIt,
  ...
  if (EntryW.state() == ExitW.state()) {
    ...
    // The `?:` arm-split mark is an annotation on one edge, not a state:
    // it is spent at the join of the arms it marks, so a merged fact never
    // carries it. Keeping the entry side's object mark-and-all made the
    // exemption follow whichever arm the walk reached first.
    else if (EntryW.armSplit() && Ctx.canModify())
      EntrySet.replaceFact(FactMan, EntryW,
                           EntryW.withState(FactMan, EntryW.state()));
    return;
  }
  ...
  if (EntryW.provedNotHeld() || ExitW.provedNotHeld()) {
    ...
    // the failure record comes with its side's negative fact, the
    ...
    const TryFactEntry &NotHeldW = EntryNotHeld ? EntryW : ExitW;
    const FactEntry *NotHeldNeg =
        (EntryNotHeld ? EntrySetOrig : ExitSet).findDefinite(FactMan, !ExitW);
    const bool Allowed = NotHeldNeg && sameOriginGate(Ctx.EntryLEK, &NotHeldW);
    if (Ctx.isLoopJoin()) {
...
void LocksetJoin::joinDefinitePair(FactSet::iterator EntryIt, FactID Fact,
  ...
  // The two kinds one cross-kind try-acquire promises, each proved on its
  // own side by that call's try fact of that kind: not two acquisitions in
  // one scope, but one acquisition whose kind the result selected. What
  // survives not knowing which is the weaker guarantee, taken silently --
  // reporting a conflict here named the same call twice, in a note at the
  // location it was warning about.
  if (crossKindPairOfOneCall(EntryFact, ExitFact)) {
    if (Ctx.canModify() && ExitFact.kind() == LK_Shared)
      *EntryIt = Fact;
    return;
  }
...
/// The same capability listed under opposite success values in the same
/// lock kind is acquired regardless of the call's result: move it out of
/// that kind's polarity groups into that kind's unconditional group,
/// leaving every remaining capability recorded under exactly one polarity
/// per kind. handleCall() adds the unconditional groups to the lockset,
/// with the diagnostic. A capability listed twice under the same polarity
/// in both kinds keeps both; the acquisition itself decides what to do
/// with the second (addTryLock()). A cross-kind pairing (exclusive on
/// success, shared on failure) is not degenerate either: it promises a
/// hold of one kind on each outcome, and stays recorded under both
/// polarities as one conditional acquisition per kind, each resolved on
/// its own attribute's edge (getEdgeLockset()).
...
void ThreadSafetyAnalyzer::reconcileTryAcquireCaps(const Expr *Exp,
  ...
  auto CoversResultDomain = [&](const CapabilityExpr &M, LockKind Kind) {
    ...
    // acquisition on any nonzero result (CapProfile::AnyNonzero) -- in
    // this kind: what the other kind's attributes promise says nothing
    // about the outcomes this one leaves open.
    return !CE || BoolResult || getCapProfile(Caps, M, Kind).AnyNonzero;
  };
  auto CollectAcquiredOnFailureToo =
      [&](const CapExprSet &Truthy, const CapExprSet &Falsy,
          CapExprSet &Unconditional, LockKind Kind) {
        for (const auto &M : Truthy)
          if (Falsy.contains(M) && CoversResultDomain(M, Kind))
            Unconditional.push_back_nodup(M);
      };
  CollectAcquiredOnFailureToo(Caps.TruthyExclusive, Caps.FalsyExclusive,
                              Caps.UnconditionalExclusive, LK_Exclusive);
  CollectAcquiredOnFailureToo(Caps.TruthyShared, Caps.FalsyShared,
                              Caps.UnconditionalShared, LK_Shared);
  // A capability promised regardless of the result in *both* kinds is one
  // unconditional acquisition, of the stronger kind: the exclusive promise
  // holds either way and subsumes the shared one, and reporting the
  // degenerate annotation twice for one capability says nothing more.
  CapExprSet SubsumedShared;
  llvm::erase_if(Caps.UnconditionalShared, [&](const CapabilityExpr &M) {
    if (!Caps.UnconditionalExclusive.contains(M))
      return false;
    SubsumedShared.push_back_nodup(M);
    return true;
  });
  if (Caps.UnconditionalExclusive.empty() && Caps.UnconditionalShared.empty())
    return;
  // Each kind's groups lose only what that kind promised regardless: the
  // other kind's conditional acquisition of the same capability stands on
  // its own attribute's edge.
  auto DropRegardless = [](CapExprSet &Set, const CapExprSet &Unconditional) {
    llvm::erase_if(Set, [&](const CapabilityExpr &M) {
      return Unconditional.contains(M);
    });
  };
  DropRegardless(Caps.TruthyExclusive, Caps.UnconditionalExclusive);
  DropRegardless(Caps.FalsyExclusive, Caps.UnconditionalExclusive);
  DropRegardless(Caps.TruthyShared, Caps.UnconditionalShared);
  DropRegardless(Caps.FalsyShared, Caps.UnconditionalShared);
  // A shared promise the exclusive one subsumed leaves its groups too: it
  // is part of the same unconditional acquisition, not a conditional
  // acquisition beside it. A shared promise on one polarity only is not
  // subsumed and stays.
  DropRegardless(Caps.TruthyShared, SubsumedShared);
  DropRegardless(Caps.FalsyShared, SubsumedShared);
}
...
ThreadSafetyAnalyzer::recordTryAcquireCall(const Expr *Exp, const NamedDecl *D,
  ...
  for (const auto &[A, Truthy, AttrCode] : Attrs) {
    ...
    for (const auto &M : AttrCaps) {
      ...
      if (!Truthy)
        continue;
      const LockKind AttrKind = A->isShared() ? LK_Shared : LK_Exclusive;
      if (!Code) {
        if (llvm::none_of(Caps.TruthyAny, [&](const auto &Recorded) {
              return Recorded.second == AttrKind && Recorded.first.equals(M);
            }))
          Caps.TruthyAny.emplace_back(M, AttrKind);
      } else if (llvm::none_of(Caps.ExactCodes, [&](const auto &Recorded) {
                   return llvm::APSInt::isSameValue(Recorded.Code, *Code) &&
                          Recorded.Kind == AttrKind && Recorded.Cap.equals(M);
                 }))
        Caps.ExactCodes.push_back({M, AttrKind, *Code});
    }
  }
